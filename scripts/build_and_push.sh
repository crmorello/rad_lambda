#!/bin/bash
# Build the arm64 lambda container image and push it to ECR. Image only: no
# lambda is touched. Roll a pushed tag out with scripts/deploy.sh.
#
...
# and an aws CLI with push permissions on the repo.
...
echo
echo "deploy it:  scripts/deploy.sh all $TAG     (or radar | tiles | obs)"
